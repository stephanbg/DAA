...
void ALU::EjecutarPrograma(const MemoriaPrograma& kMemoriaPrograma,
  ...
  while (pc_ < kSizeMemoriaPrograma) {
    ...
    if (!ValidarInstruccion(cada_instruccion)) {
      throw (pc_);
    }
    if (cada_instruccion.TipoInstruccion() == "Constante") operacion_ = new OperacionConstante();
    else if (cada_instruccion.TipoInstruccion() == "Directa") operacion_ = new OperacionDirecta();
    else operacion_ = new OperacionIndirecta();
    operacion_->EjecutarInstruccion(memoria_datos, cada_instruccion, cinta_in, cinta_out, pc_); // Internamente actualizará el pc_
    pc_++; // Se borrará
  }
}
