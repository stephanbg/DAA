...
void ALU::EjecutarPrograma(const MemoriaPrograma& kMemoriaPrograma,
  ...
  for (int i = 0; i < kSizeMemoriaPrograma; ++i) {
    if (!ValidarInstruccion(kMemoriaPrograma.get_programa()[i])) {
      throw (i);
    }
    //EjecutarLinea();
  }
...
bool ALU::ValidarInstruccion(const Instruccion& kInstruccion) {
  ...
  const std::regex kPatron1("^=[-+]?\\d+[,.]?\\d+$"); // Empieza por '=' y acaba por uno o más dígitos
  ...
  if (kInstruccion.get_instruccion()[0] == "LOAD" ||
      kInstruccion.get_instruccion()[0] == "ADD" ||
      kInstruccion.get_instruccion()[0] == "SUB" ||
      kInstruccion.get_instruccion()[0] == "MUL" ||
      kInstruccion.get_instruccion()[0] == "DIV") {
    vector_patrones.push_back(kPatron1);
    vector_patrones.push_back(kPatron2);
    vector_patrones.push_back(kPatron3);
  } else if (kInstruccion.get_instruccion()[0] == "STORE") {
    vector_patrones.push_back(kPatron2);
    vector_patrones.push_back(kPatron3);
  } else if (kInstruccion.get_instruccion()[0] == "READ") {
    vector_patrones.push_back(kPatron3);
    vector_patrones.push_back(kPatron4);
  } else if (kInstruccion.get_instruccion()[0] == "WRITE") {
    vector_patrones.push_back(kPatron1);
    vector_patrones.push_back(kPatron3);
    vector_patrones.push_back(kPatron4);
  } else if (kInstruccion.get_instruccion()[0] == "JUMP" ||
             kInstruccion.get_instruccion()[0] == "JZERO" ||
             kInstruccion.get_instruccion()[0] == "JGTZ") {
    vector_patrones.push_back(kPatron5);
  } else if (kInstruccion.get_instruccion()[0] == "HALT") {
